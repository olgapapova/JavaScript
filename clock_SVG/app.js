function constructClock() {
    ...
    let svgClock=document.getElementById("svgEl");
    svgClock.style.width=inputVal+'px';
    svgClock.style.height=inputVal+'px';
    svgClock.style.display='block';
    let inp=document.getElementsByTagName('input');
    for (let i=0; i<inp.length; i++) 
       inp[i].style.display='none';
    let centerClockX=inputVal/2;
    let centerClockY=inputVal/2;
   ...
   for (let i=1; i<=12; i++) {
    ...
    spanEl.className='CLOCK_N';
    
    spanEl.style.top=((inputVal/100*25)-spanEl.offsetHeight/2)+'px';
    spanEl.style.left=((inputVal/100*34)-spanEl.offsetWidth/2)+'px';
    spanEl.style.fontSize=spanFontSize+'px';
    document.body.appendChild(spanEl);

    let handHours=document.getElementById('handH');
